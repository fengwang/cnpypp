...
int main()
{
...

    {
        ...
        auto constexpr new_size = new_shape[0] * new_shape[1] * new_shape[2]; // std::accumulate(new_shape.begin(), new_shape.end(), 1,
                                                                              // std::multiplies<size_t>());
        auto const complete_new_data = std::invoke(
            [new_size]()
            {
        ...
        if (!std::equal(complete_new_data.cbegin(), complete_new_data.cend(), loaded_data))
        {
            std::cerr << "error in line " << __LINE__ << std::endl;
            return EXIT_FAILURE;
        }
    ...
    {
        std::vector<std::tuple<int32_t, int8_t, int16_t>> const tupleVec{{0xaaaaaaaa, 0xbb, 0xcccc}, {0xdddddddd, 0xee, 0xffff}, {0x99999999, 0x88, 0x7777}};

        cnpypp::npy_save("structured.npy", {"a", "b", "c"}, tupleVec.begin(), {tupleVec.size()});
    ...
    // std::array written as structured type
    {
        ...
        std::vector<std::array<int8_t, 2>> const arrVec{{0x11, 0x22}, {0x33, 0x44}, {0x55, 0x66}};

        cnpypp::npy_save("structured2.npy", {"a", "b"}, arrVec.begin(), {arrVec.size()});

        // load memory-mapped
        cnpypp::NpyArray arr = cnpypp::npy_load("structured2.npy", true);
        auto r = arr.tuple_range<int8_t, int8_t>();

        if (!std::equal(arrVec.begin(), arrVec.end(), r.begin(), [](auto const &a, auto const &b) { return a[0] == std::get<0>(b) && a[1] == std::get<1>(b); }))
        {
            std::cerr << "error in line " << __LINE__ << std::endl;
            return EXIT_FAILURE;
        }
